...
def get_best_airport_for_country(country_code:str):
    ...
    for iata, airport in AIRPORTS.items():
        ...
        canditates.append((score, iata))

    if not canditates:
        return None

    canditates.sort(reverse=True)
    return canditates[0][1]
...
def search_flights(query: str, limit: int = 10):
    ...
    if "error" in data:
        error = data["error"]
        # AviationStack uses 'info' for the error description, not 'message'
        err_code = error.get("code", "Unknown")
        err_type = error.get("type", "Unknown")
        err_msg = error.get("info", "No additional info provided")
